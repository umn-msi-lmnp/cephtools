...
$(if [[ ${delete_empty_dirs} -eq 0 ]]; then
    ...
    echo "temp_marker=\"${script_prefix}.empty_dir_marker\""
    ...
    echo "        relative_path=\"\${empty_dir#${path}}\""
    echo "        # Remove leading slash if present"
    echo "        relative_path=\"\${relative_path#/}\""
    echo "        # Construct bucket destination path"
    echo "        bucket_path=\"${remote}:${bucket}/${path#/}/\$relative_path/.cephtools_empty_dir_marker\""
...
$(if [[ ${delete_empty_dirs} -eq 0 ]]; then
    ...
    echo "    --log-level INFO"
    echo ""
    echo "# Remove marker files from restored directories (they preserve the empty dirs but aren't needed locally)"
